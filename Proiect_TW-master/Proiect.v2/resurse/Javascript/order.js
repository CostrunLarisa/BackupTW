...
onLoadNumarItem();
comanda();
if(document.getElementsByClassName("bara_prod"))
    {
             initiate();
          function initiate(){
    ...
    var k=0;
    var i=0;
    for(k;k<obj.length;k++)obj[k].onclick=function(e){
            var ok=0;
            for(var val of obj[i].classList)
            {
                if(val=="sterge")ok=1;
            }
            if(ok==0 && obj[i])obj[i].className+=" sterge";
	...
	if(x && cos){
			...
			Object.values(x).map(item => {
				...
				if(nrCos!=0)
				{
                    ...
                    finalizare.onclick=setTimeout(ascunde,10000);
                    function ascunde(){
                        alert("Grabiti-va!Unele produse nu se vor mai afla in stoc!");
                    }
					...
					finalizare.onclick=function(){
						...
						localStorage.removeItem("numarItem");
					}
				}
